...
const createAchat = (req, res) => {
  pool.getConnection((err, connection) => {
    ...
    const {
      ...
      qte_Magasin,
      id_Article,
    } = req.body; // Added check_Delivery
    connection.query(
      "INSERT INTO achatempo (code, code_Projet, designation, qte_En_Stock, quantite, nom_Projet, date, check_Delivery, code_Achat, user_Dmd, qte_Reçu, qte_Magasin , id_Article) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
      [
        code,
        code_Projet,
        designation,
        ...
        qte_Magasin,
        id_Article,
      ], // Included check_Delivery in values
      (err, result) => {
        connection.release();
...
const deleteAchat = (req, res) => {
  pool.getConnection((err, connection) => {
    ...
    connection.query(
      "DELETE FROM achatempo WHERE id_Achat = ?",
      [id_Achat],
      (err, result) => {
        connection.release();
        if (err) return res.status(500).send(err);
