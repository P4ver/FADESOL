...
const obtenirClientID = (req, res)=>{
    pool.getConnection((err, connection)=>{
        ...
        connection.query('SELECT * FROM clients WHERE id_Client = ?', [req.params.id], (err, rows)=>{
            connection.release()
            if (err) throw err
            console.log(rows)
            res.send(rows)
...
const ajouterClient = (req, res)=>{
    pool.getConnection((err, connection)=>{
        ...
        connection.query("INSERT INTO clients SET ?", [req.body], (err, rows)=>{
            connection.release()
            if (err) throw err
...
const modifierClient = (req, res)=>{
    pool.getConnection((err, connection)=>{
        ...
        connection.query("UPDATE clients SET nom_Client = ?, adresse_Client = ?, tel_Client = ?  WHERE id_Client = ?", [ nom_Client, adresse_Client, tel_Client, id_Client],(err, rows)=>{
            connection.release()
            if (err) throw err
...
const supprimerClient = (req, res) =>{
    pool.getConnection((err, connection)=>{
        if (err) throw err
        console.log("connection as id", connection.threadId)
        connection.query("DELETE FROM clients WHERE id_Client = ?", [req.params.id],(err, rows)=>{
            connection.release()
            if(err) throw err
            console.log(rows)
