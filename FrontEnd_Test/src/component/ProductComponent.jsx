...
const downloadCombinedPDF = async (numArticle, product) => {
    ...
    if (barcodeCanvas && qrCodeCanvas) {
        ...
        // Calculate dimensions for placing images side by side
        const imageWidth = pdfWidth / 2 - 30; // Reduce size for margin
        const imageHeight = imageWidth * 0.75; // Assuming aspect ratio

        // Header
        pdf.setFontSize(18);
        pdf.setFont('helvetica', 'bold');
        pdf.text('Détails du produit', pdfWidth / 2, 20, { align: 'center' });
        
        // Add company name
        pdf.setFontSize(14);
        pdf.setFont('helvetica', 'normal');
        pdf.text('FADESOLE POWER SOLUTIONS', pdfWidth / 2, 30, { align: 'center' });
        ...
        // Add barcode image
        pdf.addImage(barcodeImgData, 'PNG', 15, 45, imageWidth, imageHeight);

        // Add QR code image
        pdf.addImage(qrCodeImgData, 'PNG', pdfWidth / 2 + 15, 45, imageWidth, imageHeight);

        // Add product size
        pdf.setFontSize(12);
        // pdf.text(`Size: ${product.size}`, pdfWidth / 2, imageHeight + 60, { align: 'center' });

        // Decorative box around images
        pdf.setDrawColor(0);
        pdf.setLineWidth(0.1);
        pdf.rect(10, 40, pdfWidth - 20, imageHeight + 20);

        // Footer
        pdf.setFontSize(10);
        pdf.text('Generated by FADESOL', pdfWidth / 2, pdfHeight - 10, { align: 'center' });
