...
//             placeholder='Enter Numero article'
//             onChange={(e) => handleChange(index, 'demandeCode', e.target.value)}
//             className="w-1/6 px-2 py-1 border-none"
...
//                 </TableCell>
//                 <TableCell>
//                   {productData.find(demande => demande.Numéro_Article === line.demandeCode)?.Description_Article || ''}
//                   </TableCell>
//                 <TableCell>
//                   {projetData.find(projet => projet.code_Projet == line.projetCode)?.nom_Projet || ''}
//                 </TableCell>
//                 <TableCell>
//                   <input
//                     type="number"
...
//         <thead>
//           <tr>
//             <th className="border px-4 py-2">Numero Article</th>
...
const Entree = () => {
  ...
  const handleSubmit = async () => {
    try {
      ...
      for (const line of lines) {
        if (line.demandeCode && line.projetCode && line.quantite) {
          const product = productData.find(demande =>
            demande.Numéro_Article === line.demandeCode || demande.code_Barre === line.demandeCode
          );
          const achatPayload = {
            code: line.demandeCode,
            designation: product?.Description_Article || '',
            quantite: parseInt(line.quantite, 10),
            code_Projet: line.projetCode,
            nom_Projet: projetData.find(projet => projet.code_Projet === line.projetCode)?.nom_Projet || '',
            check_Delivery: false,
          ...
          {lines.map((line, index) => (
            <tr key={index}>
              ...
              <td className="border px-4 py-2">
                <input
                  type="text"
                  value={productData.find(demande =>
                    demande.Numéro_Article === line.demandeCode || demande.code_Barre === line.demandeCode
                  )?.Description_Article || ''}
                  className="w-full px-2 py-1 border-none"
                  disabled
